...
def crawl_documentation(
    ...
    response = firecrawl.crawl(
        url, limit=5, scrape_options=ScrapeOptions(formats=["markdown", "html"])
    )
...
def setup_agents(openai_api_key: str):
    ...
    processor_agent = Agent(
        ...
        instructions="""You are a helpful documentation assistant. Answer the user's question clearly and concisely. Include relevant examples when available. Cite source URLs. Keep responses natural and conversational, suitable for voice output.""",
        model="gpt-4o-mini",  # Changed to mini for lower token usage
    )

    tts_agent = Agent(
        name="Text-to-Speech Agent",
        instructions="""Convert the documentation response into natural speech. Maintain proper pacing, handle technical terms clearly, keep tone professional but friendly.""",
        model="gpt-4o-mini",
    )
...
async def process_query(
    ...
    try:
        ...
        search_response = client.query_points(
            ...
            limit=2,  # Reduced from 3 to 2 results
            with_payload=True,
        )
        ...
        for result in search_results:
            ...
            content = content[:2000]  # Limit each result to 2000 chars
            context += f"From {url}:\n{content}\n\n"

        context += f"User Question: {query}\n\n"
        context += "Provide a clear, concise answer suitable for voice output."

        # Truncate the final context to avoid exceeding token limits
        context = truncate_context(context)

        processor_result = await Runner.run(processor_agent, context)
        processor_response = processor_result.final_output

        # Skip the TTS agent step to reduce token usage
